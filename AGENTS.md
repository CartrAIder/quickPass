...
- Superpowers 스킬을 자동으로 사용하지 않는다.
- 사용자의 요청에 `@Superpowers`가 명시적으로 포함된 경우에만 Superpowers를 사용한다.
- `@Superpowers`가 없는 요청은 일반 Codex 작업 방식으로 처리한다.
- 관련성이 있어 보인다는 이유만으로 brainstorming, planning 등의 Superpowers 스킬을 자동 호출하지 않는다.
- Superpowers를 사용하더라도 이미 확정된 프로젝트 기획, 요구사항, 아키텍처는 고정된 조건으로 취급한다.
- 사용자가 명시적으로 재검토를 요청하지 않는 이상 이미 결정된 내용을 다시 기획하거나 브레인스토밍하지 않는다.